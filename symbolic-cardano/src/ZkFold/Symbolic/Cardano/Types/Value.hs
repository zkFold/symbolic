...
deriving instance (KnownNat n, KnownRegisters context 64 Auto, Symbolic context) => SymbolicData (Value n context)
...
instance (Haskell.Ord (AssetName context), Haskell.Ord (PolicyId context), Symbolic context) => Semigroup (Value n context) where
  (<>) (Value va) (Value vb) = Value $ unsafeToVector $ Map.toList $ Map.unionWith (+) (Map.fromList (fromVector va)) (Map.fromList (fromVector vb))

instance (Haskell.Ord (AssetName context), Haskell.Ord (PolicyId context), Symbolic context) => Monoid (Value n context) where
  mempty = Value $ unsafeToVector []

instance
  (Haskell.Ord (AssetName context), Haskell.Ord (PolicyId context), Symbolic context)
  => AdditiveSemigroup (Value n context)
  where
  (+) = (<>)

instance (Haskell.Ord (AssetName context), Haskell.Ord (PolicyId context), Symbolic context) => AdditiveMonoid (Value n context) where
  zero = mempty
