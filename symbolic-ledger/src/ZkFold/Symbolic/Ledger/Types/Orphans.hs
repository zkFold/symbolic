...
instance
  Symbolic context
  => Hashable (HashSimple context) (CompatData FieldElement context)
  where
  hasher = hashFn

-- TODO: Can we move these Aeson instances to their respective type-definition module?
instance FromJSON (FieldElement (CompatContext RollupBF)) where
  parseJSON v = fromConstant @Integer <$> parseJSON v

instance ToSchema (CompatData FieldElement RollupBF) where
  declareNamedSchema _ = declareNamedSchema (Proxy @RollupBF)

instance FromJSON (SBool.Bool (CompatContext RollupBF)) where
...
instance ToSchema (CompatData SBool.Bool RollupBF) where
...
instance forall n r. (KnownRegisterSize r, KnownNat n) => ToJSON (Int n r RollupBF) where
  toJSON = toJSON . toConstant

instance forall n r. (KnownRegisterSize r, KnownNat n) => FromJSON (Int n r RollupBF) where
  parseJSON v = fromConstant @Integer <$> parseJSON v

instance forall n r. (KnownNat n, Typeable r) => ToSchema (Int n r RollupBF) where
  declareNamedSchema _ = declareNamedSchema (Proxy @Integer)

-- Matches JSON encoding of UInt as a natural number.
instance forall n r. (KnownNat n, Typeable r) => ToSchema (UInt n r RollupBF) where
  declareNamedSchema _ = declareNamedSchema (Proxy @Natural)

instance
  forall a
   . KnownFFA a 'Auto RollupBF
  => ToJSON (FFA a 'Auto RollupBF)
  where
...
instance
  forall curve a
   . KnownFFA a 'Auto RollupBF
  => ToJSON (AffinePoint curve (FFA a 'Auto) RollupBF)
  where
  toJSON p =
...
instance
  forall curve a
   . KnownFFA a 'Auto RollupBF
  => FromJSON (AffinePoint curve (FFA a 'Auto) RollupBF)
  where
  parseJSON =
...
instance
  forall a
   . KnownFFA a 'Auto RollupBF
  => ToSchema (AffinePoint (TwistedEdwards "jubjub") (FFA a 'Auto) RollupBF)
  where
  declareNamedSchema _ = do
    xyRef <- declareSchemaRef (Proxy @(FFA a 'Auto RollupBF))
    let schema =
          Haskell.mempty
            & type_ ?~ OpenApiObject
