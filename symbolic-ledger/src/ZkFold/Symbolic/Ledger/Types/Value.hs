...
--
-- Stored as a 'PolyBounded' 64-bit field element so that all constraints
-- generated by arithmetic and comparison operations are polynomial.  This
-- makes 'AssetQuantity' safe to use inside Protostar IVC step circuits.
type AssetQuantity context = PolyBounded 64 context

-- | Previously constrained the number of registers for 'Int 64 Auto'.
-- Now trivially satisfied because 'PolyBounded' uses a single field element.
class KnownRegistersAssetQuantity context

instance KnownRegistersAssetQuantity context
