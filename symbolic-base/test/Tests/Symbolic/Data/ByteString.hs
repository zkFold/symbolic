...
import Tests.Symbolic.Data.Common.V2 (specConstantRoundtripV2, specSymbolicFunction1V2, specSymbolicFunction0V2, specSymbolicFunction2V2)
import ZkFold.Symbolic.Compat (CompatContext(CompatContext))
import Data.Binary (Binary)

eval :: forall a n. (Arithmetic a, Binary a) => ByteString n (Elem a) -> ByteString n a
eval (ByteString bits) = ByteString $ exec (CompatContext bits)
...
testGrow
  ...
  => PrimeField (Zp p)
  => KnownNat m
  => Resize (ByteString n (Elem (Zp p))) (ByteString m (Elem (Zp p)))
  => Spec
...
specByteString' = do
  ...
  describe ("ByteString" ++ show n ++ " specification") $ do
    specConstantRoundtripV2 @(Zp p) @(ByteString n) ("ByteString" ++ show n) "Natural" (toss m)
    specSymbolicFunction1V2 @(Zp p) @(ByteString n) "identity" id
    specSymbolicFunction0V2 @(Zp p) @(ByteString n) "true" true
    specSymbolicFunction0V2 @(Zp p) @(ByteString n) "false" false
    specSymbolicFunction2V2 @(Zp p) @(ByteString n) "bitwise OR" (||)
    specSymbolicFunction2V2 @(Zp p) @(ByteString n) "bitwise XOR" xor
    specSymbolicFunction2V2 @(Zp p) @(ByteString n) "bitwise AND" (&&)
    specSymbolicFunction1V2 @(Zp p) @(ByteString n) "bitwise NOT" not
