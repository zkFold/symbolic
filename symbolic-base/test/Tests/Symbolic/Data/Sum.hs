...
instance Arithmetic a => Q.Function (CompatData FieldElement a) where
...
instance Arithmetic a => Q.CoArbitrary (CompatData FieldElement a) where
...
specOneOf' = describe (show (typeAt @(OneOf ts a)) <> " spec") do
  it "preserves sum" \(Q.Fn f) e ->
    matchOneOf @(CompatData FieldElement) @ts @a (embedOneOf e) f Q.=== f e

type TestFun t a = t a -> CompatData FieldElement a
...
specOneOf
  ...
  => Spec
specOneOf = do
  specOneOf' @a @'[CompatData FieldElement]
  specOneOf' @a @'[U1, CompatData FieldElement]
  specOneOf' @a @'[CompatData FieldElement, ByteString 16]
  specOneOf' @a @'[ByteString 16, CompatData FieldElement, UInt 32 Auto]
...
data OneOf3 c
  = BS (ByteString 16 c)
  | FE (CompatData FieldElement c)
  | UD (UInt 32 Auto c)
  deriving (Generic, Generic1, Show)

instance Symbolic c => Q.Arbitrary (OneOf3 c) where
  arbitrary = Q.oneof [BS <$> Q.arbitrary, FE <$> Q.arbitrary, UD <$> Q.arbitrary]

instance Arithmetic a => Q.CoArbitrary (OneOf3 a)

instance Arithmetic a => Q.Function (OneOf3 a)

specSumOf
  :: forall a
   . (Arithmetic a, Show a, Typeable a, KnownNat (NumberOfRegisters a 32 Auto)) => Spec
specSumOf = do
  specSumOf' @a @(Only (CompatData FieldElement))
  specSumOf' @a @(Might (CompatData FieldElement))
  specSumOf' @a @(CompatData FieldElement :+: ByteString 16)
  specSumOf' @a @OneOf3
