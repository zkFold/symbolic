...
instance (Arithmetic a, KnownNat n, KnownRegisterSize r) =>
    Q.Function (UInt n r (Interpreter a)) where
    function = Q.functionMap toConstant fromConstant
...
instance (Arithmetic a, KnownNat n, KnownRegisterSize r) =>
    Q.CoArbitrary (UInt n r (Interpreter a)) where
    coarbitrary = Q.coarbitrary . toConstant

specOneOf' ::
    forall a ts.
    ( Show a, Typeable a, Q.Arbitrary a, Typeable ts, Embed ts (Interpreter a), Show (Eithers ts)
    , Q.Arbitrary (Eithers ts), Q.CoArbitrary (Eithers ts), Q.Function (Eithers ts)
    ) => Spec
...
specSumOf' ::
    forall a t.
    ( Show a, Typeable a, Q.Arbitrary a, Generic t, Show t, Typeable t
    , Q.Arbitrary t, Q.CoArbitrary t, Q.Function t, Injects (Rep t) (Interpreter a)) => Spec
...
specOneOf ::
    forall a.
    ( Arithmetic a, Show a, Typeable a, Q.Arbitrary a
