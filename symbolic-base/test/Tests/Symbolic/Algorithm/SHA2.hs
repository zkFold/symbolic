...
testAlgorithm file = do
  ...
  describe description $
    forM_ testCases $ \(bits, input, hash) -> do
      ...
      it bitMsgS $ toConstant (sha2Var @algorithm @element @10000 $ fromNatural bits input) `shouldBe` hash
 where
  description :: String
...
eval (ByteString bits) = exec (CompatContext bits)
