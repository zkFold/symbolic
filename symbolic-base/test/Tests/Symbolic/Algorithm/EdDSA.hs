...
specEdDSA = describe "EdDSA verification (Jubjub, Poseidon Hash)" $ do
  it "verifies a correctly formed signature for simple case" $ do
    ...
    counterexample
      ( "g = "
          <> show (SymAffine.affinePoint g)
          <> "\nprivKey = "
          <> show privKey
          <> "\npubKey = "
          <> show (SymAffine.affinePoint pubKey)
          <> "\nr = "
          <> show r
          <> "\nrPoint = "
          <> show (SymAffine.affinePoint rPoint)
          <> "\ns = "
          <> show s
          <> "\nlhs = "
          <> show (SymAffine.affinePoint lhs)
          <> "\nrhs = "
          <> show (SymAffine.affinePoint rhs)
          <> "\nok = "
          <> show ok
          <> "\nhpubKey' = "
