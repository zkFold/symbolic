...
problematicPolynomials =
  ...
  , let v1 = toVar "y\ETX^\246\226\195\154S\130M\tL\146y\248\201\162\220 \237n6p\bC\151\186\241\US\136\225\139"
        v2 = toVar "~\180\185\222\SOH!\t\254\155\v\SI\187\&9\227\163|^\168Z\184Q\129\rN\218\SYN\GSp\189\139~^"
     in poly [(one, mono $ fromList [(v1, 1), (v2, 1)])]
  , poly [(one, mono $ fromList [(toVar "v1", 1), (toVar "v2", 1)])]
  , poly [(one, mono $ fromList [(toVar "v1", 1), (ConstVar one, 1)])]
  ]
...
propPlonkupRelationHolds PlonkupRelation {..} w zeta =
  let (w1, w2, w3) = witness w
      ...
      !f_zeta =
        toPolyVec $ V.zipWith3 (\lk ti ai -> bool ti ai (lk == one)) (fromPolyVec qK) (fromPolyVec t_zeta) (fromPolyVec f_zeta')
          :: PolyVec a n
...
specPlonkup = do
  describe "Plonkup specification" $ do
    describe "Conversion to Plonk constraints and back" $ do
      it "produces equivalent polynomials" $
        property $
          withMaxSuccess 10 $
            propPlonkConstraintConversion @(ScalarFieldOf BLS12_381_G1_JacobianPoint)
      ...
      it "'ConstVar a' does not cause exceptions " $
        property $
          withMaxSuccess 10 $ \v ->
            fromPlonkConstraint (toPlonkConstraint @(ScalarFieldOf BLS12_381_G1_JacobianPoint) @(Vector 1) (var $ ConstVar v))
              == var (ConstVar v)
    ...
    describe "Plonkup relation" $ do
      it "should hold" $
        property $
          withMaxSuccess 10 $
            propPlonkupRelationHolds @(Vector 2) @(Vector 3) @32 @(ScalarFieldOf BLS12_381_G1_JacobianPoint)
