...
instance Storable a => Storable (PlonkupWitnessData a) where
  ...
  peek ptr' = do
    [w1, w2, w3] <-
      (snd <$>) . tail <$> sequence (scanl (peekWithOff @[a] (castPtr ptr')) (pure (0, undefined)) (replicate 3 undefined))
...
instance Storable a => Storable (a, a, a, a) where
  ...
  peek ptr = do
    [a, b, c, d] <-
      (snd <$>) . tail <$> sequence (scanl (peekWithOff @a (castPtr ptr)) (pure (0, undefined)) (replicate 4 undefined))
    pure (a, b, c, d)
...
instance Storable a => Storable (PlonkupCircuit a) where
  sizeOf PlonkupCircuit {..} =
    sum
      [ sizeOf $ naturalToUInt64 circuitSize
      , sizeOf witness
      , sizeOf selectors
      , sizeOf table
      , sizeOf $ mapTup4 naturalToUInt64 <$> copyConstraints
      ]
