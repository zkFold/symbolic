...
data PlonkupSetup i n w l g1 g2 pv = PlonkupSetup
...
instance
        ...
        ) => Show (PlonkupSetup i n w l g1 g2 pv) where
    show PlonkupSetup {..} =
        "Setup: "
        ++ show omega ++ " "
        ++ show k1 ++ " "
...
plonkupSetup :: forall i n w l g1 g2 gt ts pv .
    ( Representable i
    , Representable w
    , Representable l
    , Foldable w
    ...
    ) => Plonkup i n w l g1 g2 ts pv -> PlonkupSetup i n w l g1 g2 pv
plonkupSetup Plonkup {..} =
    let gs = toV gs'
        h0 = pointGen

        relation@PlonkupRelation{..} = fromJust $ toPlonkupRelation ac :: PlonkupRelation i n w l (ScalarFieldOf g1) pv
