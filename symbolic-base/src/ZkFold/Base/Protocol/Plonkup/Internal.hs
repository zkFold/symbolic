...
data Plonkup i (n :: Natural) w l g1 g2 transcript pv = Plonkup {
...
instance (Show (ScalarFieldOf g1), Show (Rep i), Show1 w, Show1 l, Ord (Rep i), Show g1, Show g2) => Show (Plonkup i n w l g1 g2 t pv) where
    show Plonkup {..} =
...
instance
  ...
  , Arbitrary (ArithmeticCircuit (ScalarFieldOf g1) i (w :*: l))
  ) => Arbitrary (Plonkup i n w l g1 g2 t pv) where
    arbitrary = do
        ac <- arbitrary
        x <- arbitrary
