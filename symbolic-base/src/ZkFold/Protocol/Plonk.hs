...
data Plonk i o p (n :: Natural) g1 g2 transcript pv = Plonk {
...
fromPlonkup ::
    ...
    ) => Plonkup i o p n g1 g2 ts pv -> Plonk i o p n g1 g2 ts pv
fromPlonkup Plonkup {..} = Plonk { ac = desugarRanges ac, ..}

toPlonkup :: Plonk i o p n g1 g2 ts pv -> Plonkup i o p n g1 g2 ts pv
toPlonkup Plonk {..} = Plonkup {..}

instance ( Show1 o, Show1 p, Show (Rep i), Show (ScalarFieldOf g1)
         , Ord (Rep i), Show g1, Show g2) => Show (Plonk i o p n g1 g2 t pv) where
    show Plonk {..} =
...
instance ( Arithmetic (ScalarFieldOf g1), Binary (ScalarFieldOf g1)
         , Binary (Rep i), Ord (Rep i), Representable i
         , Arbitrary (Plonkup i o p n g1 g2 t pv))
        => Arbitrary (Plonk i o p n g1 g2 t pv) where
    arbitrary = fromPlonkup <$> arbitrary

instance forall i o p n g1 g2 gt (ts :: Type) pv .
        ( NonInteractiveProof (Plonkup i o p n g1 g2 ts pv)
        ...
        ) => NonInteractiveProof (Plonk i o p n g1 g2 ts pv) where
    type Transcript (Plonk i o p n g1 g2 ts pv)  = ts
    ...
    setupProve ::
      Plonk i o p n g1 g2 ts pv -> SetupProve (Plonk i o p n g1 g2 ts pv)
    setupProve = setupProve @(Plonkup i o p n g1 g2 ts pv) . toPlonkup

    setupVerify ::
      Plonk i o p n g1 g2 ts pv -> SetupVerify (Plonk i o p n g1 g2 ts pv)
    setupVerify = setupVerify @(Plonkup i o p n g1 g2 ts pv) . toPlonkup

    prove ::
      SetupProve (Plonk i o p n g1 g2 ts pv) ->
      Witness (Plonk i o p n g1 g2 ts pv) ->
      (Input (Plonk i o p n g1 g2 ts pv), Proof (Plonk i o p n g1 g2 ts pv))
    prove setup witness =
        let (input, proof, _) = plonkProve @i @o @p @n @g1 @g2 @ts @pv setup witness
        in (input, proof)

    verify ::
      SetupVerify (Plonk i o p n g1 g2 ts pv) ->
      Input (Plonk i o p n g1 g2 ts pv) -> Proof (Plonk i o p n g1 g2 ts pv) -> Bool
    verify = plonkVerify @i @o @p @n @g1 @g2 @gt @ts @pv
