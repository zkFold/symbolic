...
import Data.List (find, head, map, permutations, sort, (!!), (++), tails, concatMap, all, filter)
...
toPlonkConstraint p = PlonkConstraint qm ql qr qo qc va vb vc 
  where
    fail :: P.String -> x
    fail s = P.error $ s <> ": not a plonk constraint. Monomials of the following degrees were encountered: " <> P.show (fmap (degM . P.snd) $ toList p)
