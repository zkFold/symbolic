...
import           ZkFold.Symbolic.Compiler.ArithmeticCircuit.Context (CircuitContext (..), CircuitFold (..), Constraint,
                                                                     witToVar)
...
optimize ::
    forall a o. (Arithmetic a, Binary a, Functor o) =>
...
optimize keep (CircuitContext s lf lc w f o) =
    ...
    where
        ...
        inputConstraints vs = M.fromList [
                (polyId, poly) | (inVar, v) <- M.assocs $ filterKeys keep vs,
        ...
        optimizeFold CircuitFold {..} = CircuitFold {
