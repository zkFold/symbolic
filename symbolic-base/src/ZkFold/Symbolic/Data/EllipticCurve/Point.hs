...
import GHC.Generics (Generic1, Generic, Par1 (..), U1 (..))
import ZkFold.Symbolic.Data.Class (SymbolicData)
import ZkFold.Data.HFunctor.Classes (HNFData, HEq)
...
instance BooleanOf (f c) ~ Bool c =>
  FromConstant (Elliptic.Point (f c)) (Point curve f c) where
...
instance
  ...
  => Conditional (Bool c) (Elliptic.Point (f c)) where
...
instance (Symbolic c, MultiplicativeSemigroup (f c), SymbolicEq f c) =>
  Eq (Point curve f c) where
  type BooleanOf (Point curve f c) = Bool c
  Point x y i == Point x' y' i' =
    ifThenElse (i || i') (i && i' && x' * y == x * y') ((x, y) == (x', y'))

instance (Symbolic c, SymbolicEq f c, Semiring (f c)) =>
  Zero (Point curve f c) where
  zero = viaWeierstrass (const zero) U1

instance {-# OVERLAPPABLE #-}
  (BooleanOf (f c) ~ Bool c, Scale k (WEP curve f c)) =>
  Scale k (Point curve f c) where
  scale k = viaWeierstrass (scale k . unPar1) . Par1

instance (Symbolic c, SymbolicEq f c, Field (f c)) =>
  AdditiveSemigroup (Point curve f c) where
  (+) = curry $ viaWeierstrass (uncurry (+) . unTwo) . uncurry Two

instance (Symbolic c, SymbolicEq f c, WeierstrassCurve curve (f c)) =>
  AdditiveMonoid (Point curve f c)

instance (Symbolic c, SymbolicEq f c, WeierstrassCurve curve (f c)) =>
  AdditiveGroup (Point curve f c) where
