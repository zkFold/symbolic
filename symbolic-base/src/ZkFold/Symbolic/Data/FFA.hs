...
isUIntFFAValid
  :: (Symbolic c, KnownFFA p r c) => UIntFFA p r c -> Bool c
...
isValidFFA
  :: forall p r c. (Symbolic c, KnownFFA p r c) => FFA p r c -> Bool c
...
valueFFA
  ...
  => FieldElement c -> UIntFFA p r c -> IntegralOf c
valueFFA
  (toLayout -> Par1 ni)
...
layoutFFA c = FFA (FieldElement $ fromConstant c) (UIntFFA $ layoutUInt c)
  ...
  FFA nx ux + FFA ny uy =
   ...
   where
    ...
    result =
      assert
        ( \ffa@(FFA nm um) ->
            isValid d -- boolean is indeed boolean;
              && isValidFFA ffa -- result is indeed FFA;
...
toUInt
  :: forall n p r c
   . Symbolic c
  => (KnownFFA p r c, KnownNat n, KnownRegisters c n r)
