...
import ZkFold.Symbolic.Data.V2 (SymbolicData (..), dummy)
...
data OneOf ts c = OneOf
  { discriminant :: CompatData FieldElement c
...
instance
  ( Symbolic c
  , Eq (Product ts c)
  , BooleanOf (Product ts c) ~ CompatData Bool c
  ) => Eq (OneOf ts c)
...
matchOneOf OneOf {..} f =
  case zipWith
    ...
    [] -> dummy
    (b : bs) ->
      let CompatData (FieldElement (CompatContext (G.Par1 d))) = discriminant
...
deriving newtype instance
  ( Symbolic c
  , Eq (Prod a c)
  , BooleanOf (Prod a c) ~ CompatData Bool c
  ) => Eq (Sum a c)
