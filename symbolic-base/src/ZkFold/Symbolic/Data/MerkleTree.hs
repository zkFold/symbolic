...
import ZkFold.Symbolic.Compat
import ZkFold.Symbolic.Data.Bool (Bool (..), BoolType (..), Conditional, bool, (||))
...
data MerkleTree d c = MerkleTree
  { mHash :: CompatData FieldElement c
...
instance Symbolic c => Eq (MerkleTree d c) where
  type BooleanOf (MerkleTree d c) = CompatData Bool c
  (==) = (==) `on` mHash
  (/=) = (/=) `on` mHash

instance P.Eq c => P.Eq (MerkleTree d c) where
  MerkleTree rh _ == MerkleTree rh' _ = rh P.== rh'

deriving instance P.Show c => P.Show (MerkleTree d c)
...
fromLeaves :: Symbolic c => Base.Leaves d (CompatData FieldElement c) -> MerkleTree d c
fromLeaves src@(fmap toBaseHash -> mLeaves) =
  let mHash = Base.computeRoot src in MerkleTree {..}

toLeaves :: Symbolic c => MerkleTree d c -> Base.Leaves d (CompatData FieldElement c)
toLeaves src@MerkleTree {..} =
  assertAll ((== src) . fromLeaves) $ fromBaseHash <$> mLeaves

instance
  (Symbolic c, FromConstant a (CompatData FieldElement c), Base.MerkleTreeSize d ~ n)
...
instance
  (Symbolic c, Arbitrary c, KnownNat (Base.MerkleTreeSize d))
  => Arbitrary (MerkleTree d c)
  where
  arbitrary = fromLeaves . fmap CompatData <$> arbitrary

type MerklePath d = Vector (d - 1) :.: (CompatData Bool :*: CompatData FieldElement)

type Index d = Vector (d - 1) :.: CompatData Bool
...
rootOnReplace
  :: Symbolic c
  => MerklePath d c -> CompatData FieldElement c -> CompatData FieldElement c
rootOnReplace (Comp1 path) value =
  foldl' ((. toPair) . Base.hashWithSibling) value path

data MerkleEntry d c = MerkleEntry
  { position :: Index d c
  , value :: CompatData FieldElement c
  }
  deriving (Generic, Generic1, SymbolicData)

deriving stock instance P.Show c => P.Show (MerkleEntry d c)
...
tree !! position =
 ...
 where
  ...
  fromBool :: CompatData Bool c -> BooleanOf c
  fromBool (CompatData (Bool (CompatContext (Par1 b)))) = b == one

search
  :: forall c d
   . (Symbolic c, KnownNat (d - 1))
  => (CompatData FieldElement (Witness c) -> CompatData Bool (Witness c))
  -> MerkleTree d c
...
search pred tree =
  assert (\entry -> tree `contains` fromJust entry) $
    toEntry $
      recSearch
        (fromBool . pred . CompatData . FieldElement . CompatContext . Par1)
        (mLeaves tree)
 where
  recSearch
    :: forall n b a
  ...
  fromBool :: CompatData Bool (Witness c) -> BooleanOf c
  fromBool (CompatData (Bool (CompatContext (Par1 (Witness b))))) = b == one

  toBool :: BooleanOf c -> CompatData Bool c
...
find
  ...
  => (CompatData FieldElement (Witness c) -> CompatData Bool (Witness c))
  -> MerkleTree d c
  -> Maybe (CompatData FieldElement) c
...
elemIndex
  :: forall c d
   . (Symbolic c, KnownNat (d - 1))
  => CompatData FieldElement (Witness c)
  -> MerkleTree d c
  -> Maybe (Index d) c
...
search'
  ...
  => ( forall e
       ...
       => CompatData FieldElement e -> CompatData Bool e
     )
  -> MerkleTree d c
  -> MerkleEntry d c
...
fromBaseHash :: Witness c -> CompatData FieldElement c
fromBaseHash = CompatData . FieldElement . CompatContext . Par1 . witness

toBaseHash :: CompatData FieldElement c -> Witness c
toBaseHash = Witness . unPar1 . compatContext . fromFieldElement . compatData

toBasePosition
  :: forall c d. Symbolic c => Index d c -> IntegralOf c
toBasePosition = foldl' (\x b -> double x + fromBool b) zero . unComp1
 where
  fromBool :: CompatData Bool c -> IntegralOf c
  fromBool (CompatData (Bool (CompatContext (Par1 b)))) = toIntegral b
