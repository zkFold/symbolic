...
nothing :: (SymbolicData x, Symbolic c, PayloadFunctor (Layout x (BaseField c))) =>
  Maybe x c
nothing =
  Maybe false $ fromContext $ embed $ pureRep zero

fromMaybe :: (SymbolicData x, Symbolic c, PayloadFunctor (Layout x (BaseField c))) =>
  x c -> Maybe x c -> x c
...
maybe :: (SymbolicData b, Symbolic c, PayloadFunctor (Layout b (BaseField c)))
  => b c -> (a c -> b c) -> Maybe a c -> b c
maybe d h m =
  let Maybe {..} = m
  in fromMaybe d (just $ h fromJust)

find :: forall a c t .
    (SymbolicData a, Symbolic c, PayloadFunctor (Layout a (BaseField c)), Foldable t) =>
