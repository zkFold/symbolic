...
import Data.Constraint.Nat (euclideanNat, plusCommutes, timesCommutes)
...
data UIntData n k c = MkUIntData
  ...
  deriving (Collect (ConstrainedDatum c), Eq, Generic, Generic1, SymbolicData)
...
instance {-# OVERLAPPING #-} Symbolic c => Scale (UIntData n k c) (UIntData n k c)
...
instance
  ...
  => FromConstant Natural (UIntData n k c)
  where
  fromConstant =
...
instance
  (KnownUIntData n k, FromConstant Natural c)
  => FromConstant Integer (UIntData n k c)
  where
  fromConstant =
    withDict (pow2Nat @n) (fromSemiEuclidean . fromZp @(2 ^ n) . fromConstant)
...
instance (KnownUIntData n k, Symbolic c) => Exponent (UIntData n k c) Natural where
  (^) = natPow
...
instance (KnownUIntData n k, Symbolic c) => MultiplicativeMonoid (UIntData n k c) where
