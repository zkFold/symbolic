...
feToUInt :: forall n ctx. CompatData FieldElement ctx -> UInt (BufLen n) ('Fixed (BufLen n)) ctx
feToUInt (CompatData (FieldElement (CompatContext (Par1 c)))) =
  UInt $ withDict (oneReg @n @ctx) $ V.singleton c

uintToFe :: forall n ctx. UInt (BufLen n) ('Fixed (BufLen n)) ctx -> CompatData FieldElement ctx
uintToFe (UInt v) = CompatData $ FieldElement $ CompatContext $ withDict (oneReg @n @ctx) $ Par1 $ V.item v
...
padTo6 ui = CompatData $ FieldElement $ fromCircuitF (CompatContext v) $ \bits ->
  do
  ...
  newLen =
    knownBufLen @n $
      withDiv @(BufLen n) $
        knownOneReg @n @ctx $
          knownNumWords @n @ctx $
            scale (4 :: Natural) . (uintToFe @n) . (`div` fromConstant @Natural 3) . (feToUInt @n) $
              bsLength
