...
compareWise :: forall r c f . (Symbolic c, Z.Zip f, Foldable f, KnownNat r) => c f -> c f -> Ordering c
-- ^ Given two lists of bits of equal length, compares them lexicographically.
compareWise xs ys = Ordering $ symbolic2F xs ys
    ( \us vs -> Par1 $
        case Haskell.compare (toList us) (toList vs) of
            Haskell.LT -> negate one
            Haskell.EQ -> zero
            Haskell.GT -> one
    )
    ( \is js -> Par1 <$> do
        (hasOne, hasNegOne) <- circuitDelta @r is js
        newAssigned $ \p -> p hasOne - p hasNegOne
    )
--   in
