...
newtype UInt (n :: Natural) (r :: RegisterSize) c = UInt
  ...
  deriving (Collect (ConstrainedDatum c), Eq) via (CompatData (Vec (Vector (NumberOfRegisters c n r))) c)
...
instance SymbolicData (UInt n r) where
  ...
  toLayout = uintData
  fromLayout = UInt
  interpolate c bs = UInt
    $ compatContext
    $ I.interpolate (bimap fromConstant (CompatContext . uintData) <$> bs)
    $ CompatContext (Par1 c)

isValidUInt
  :: (Symbolic c, KnownNat n, KnownRegisterSize r)
  => UInt n r c -> CompatData Bool c
...
toNative
  ...
  => UInt n r c -> CompatData FieldElement c
toNative (UInt _rs) =
  CompatData $ FieldElement $ error "TODO"
    -- symbolicF
    --   rs
...
instance (Symbolic c, KnownRegisterSize r, NumberOfBits c ~ n) => Iso (CompatData FieldElement c) (UInt n r c) where
  from a = from (from a :: ByteString n c)

instance (Symbolic c, KnownRegisterSize r, NumberOfBits c ~ n) => Iso (UInt n r c) (CompatData FieldElement c) where
  from a = from (from a :: ByteString n c)
...
instance
  ...
  => Ord (UInt n r c)
  where
  type OrderingOf (UInt n r c) = CompatData Ordering c
  ...
  UInt u1 >= UInt u2 =
    let w1 = asWords @OrdWord @regSize u1
     ...
     in CompatData $ bitwiseGE @OrdWord (CompatContext w1) (CompatContext w2)

  UInt u1 > UInt u2 =
    let w1 = asWords @OrdWord @regSize u1
        w2 = asWords @OrdWord @regSize u2
     in CompatData $ bitwiseGT @OrdWord (CompatContext w1) (CompatContext w2)
...
mulFFT x y = c
 where
  ...
  _xHat = compatContext $ fft (CompatContext x)
  _yHat = compatContext $ fft (CompatContext y)

  c :: Vector (2 ^ k) c
  c = compatContext $ ifft $ CompatContext $ error "TODO" -- fromCircuit2F xHat yHat $ \u v ->
...
instance (Symbolic c, KnownNat n, KnownRegisterSize r) => StrictConv (CompatData FieldElement c) (UInt n r c) where
  strictConv _a =
    UInt $ error "TODO"
--      symbolicF
--        a
