...
instance (PayloadFunctor f, Symbolic c) => SymbolicData (Payloaded f c) where
...
instance (PayloadFunctor f, Symbolic c) => SymbolicInput (Payloaded f c) where
  isValid = const true

instance (PayloadFunctor f, Symbolic c) => Conditional (Bool c) (Payloaded f c) where
  bool (Payloaded onFalse) (Payloaded onTrue) (Bool (witnessF -> Par1 b)) =
    Payloaded $ mzipWithRep (\f t -> t * b + (one - b) * f) onFalse onTrue

instance (PayloadFunctor f, Symbolic c) => Eq (Payloaded f c) where
