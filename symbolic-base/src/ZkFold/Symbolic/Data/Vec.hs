...
instance (Symbolic c, Traversable f, Representable f) => MultiplicativeSemigroup (Vec f c) where
    (Vec v1) * (Vec v2) = Vec $ fromCircuit2F v1 v2 $ mzipWithMRep  (\i j -> newAssigned $ ($ i) * ($ j))
...
instance (Symbolic c, Traversable f, Representable f) => AdditiveSemigroup (Vec f c) where
    (Vec v1) + (Vec v2) = Vec $ fromCircuit2F v1 v2 $ mzipWithMRep  (\i j -> newAssigned $ ($ i) + ($ j))

instance (Symbolic c, Traversable f, Representable f) => MultiplicativeMonoid (Vec f c) where
    one = fromConstant (1 :: Natural)

instance (Symbolic c, Traversable f, Representable f) => AdditiveMonoid (Vec f c) where
    zero = fromConstant (0 :: Natural)


instance (Symbolic c, Traversable f, Representable f) => AdditiveGroup (Vec f c) where
