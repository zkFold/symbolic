...
instance KnownNat n => SymbolicInput (Register n) where
  isValid (MkRegister x) =
    let bound = 2 ^ value @n
        m = fromConstant (toIntegral x `mod` fromConstant bound)
...
splitChunksR
  :: (KnownNat m, KnownNat n, KnownNat k, Symbolic c)
...
instance (KnownNat n, Symbolic c) => BoolType (Register n c) where
  false = zero
  true = MkRegister $ fromConstant (2 ^ value @n -! 1)
  not (regToFE -> x) =
