...
type Keccak algorithm context k =
  ...
  , -- So that we are dealing with "byte"strings.
    Mod k 8 ~ 0
  , Symbolic context
  , KnownRegisters context (NumberOfBits context) Auto
  , KnownNat
      ( Ceil
          (GetRegisterSize context (NumberOfBits context) Auto)
          OrdWord
      )
  )
...
keccak
  ...
  -> ByteString (ResultSizeInBits (Rate algorithm)) context
keccak bs =
...
paddingVar
  :: forall algorithm context k
   . Keccak algorithm context k
  => VarByteString k context
  -> VarByteString (PaddedLengthBits k (Rate algorithm)) context
paddingVar (VarByteString {..}) =
  withMessageLengthConstraints @k @(Rate algorithm) $
    let
      rateN = value @(Rate algorithm)
      bsLengthModRate :: UInt (NumberOfBits context) Auto context
      bsLengthModRate = from bsLength `mod` fromConstant rateN
      ...
      grown = resize bsBuffer & (`VB.shiftL` padLengthBits)
     in VarByteString paddedLengthBits (grown || padBS)
...
toBlocks msg =
  withMessageLengthConstraints @k @(Rate algorithm) $
    let byteWords =
          (toWords msg
            :: Vector (PaddedLengthBytesFromBits k (Rate algorithm))
                      (ByteString 8 context))
  ...
  withMessageLengthConstraints @k @(Rate algorithm) $
    let blockChunks
          :: Vector
               ...
               (Vector (AbsorbChunkSize algorithm)
                       (ByteString LaneWidth context)) = chunks blocks
     in foldl'
          ( \accState chunk ->
  ...
  withMessageLengthConstraints @k @(Rate algorithm) $
    let blockChunks
          :: Vector
               ...
               (Vector (AbsorbChunkSize algorithm)
                       (ByteString LaneWidth context)) = chunks blocks
        actualChunksCount :: CompatData FieldElement context =
          let absorbChunkSize = fromConstant $ value @(AbsorbChunkSize algorithm)
              numBlocks = from @_ @(UInt (NumberOfBits context) Auto context) paddedMsgLen `div` fromConstant (value @LaneWidth)
        ...
        foldl'
          ( \accState (fromZp -> ix, chunk) ->
              let state' = updateStateInAbsorption @algorithm @context chunk threshold accState
                  ixFE :: CompatData FieldElement context = fromConstant ix
               in unComp1 $
                    ifThenElse
...
theta state =
 ...
 where
  ...
  d :: Vector 5 (ByteString LaneWidth context) =
    tabulate
      ( \(fromZp -> i) ->
        (c !! P.fromIntegral (((P.fromIntegral i :: P.Integer) - 1) `mod` 5))
        `xor` rotateBitsL (c !! ((i + 1) `mod` 5)) 1
      )
...
iota
  :: forall context
   . Symbolic context
  => Natural -> Vector NumLanes (ByteString LaneWidth context)
  -> Vector NumLanes (ByteString LaneWidth context)
...
{-# INLINE reverseBits #-}
reverseBits :: forall n context. ByteString n context -> ByteString n context
reverseBits (ByteString cbs) = ByteString (reverse cbs)
