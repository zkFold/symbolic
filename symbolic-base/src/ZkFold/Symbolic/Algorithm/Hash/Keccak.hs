...
import ZkFold.Data.Vector (Vector (..), chunks, concatMap, fromVector, generate, indexed, mapWithIx, reverse, slice, unsafeSlice, unsafeToVector, (!!))
      ...
      ( \i ->
          P.foldl1
            ...
            ( case someNatVal (i) of
                SomeNat (_ :: Proxy i) ->
                  withDict (timesNat @i @5) $
                    unsafeSlice @(i * 5) @5 @NumLanes state
            )
      )
