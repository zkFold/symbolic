...
shiftUIntR u n = from @(ByteString 64 b) $ from u `shiftBitsR` n
...
xorUInt u1 u2 = from @(ByteString 64 c) $ from u1 `xor` from u2
...
blake2b_compress Blake2bCtx {h, m, t} lastBlock =
  let v' = h V.++ blake2b_iv -- init work variables
      v'' =
        v'
          V.// [ (12, (v' ! 12) `xorUInt` fromConstant (fst t)) -- low word of the offset
               , (13, (v' ! 13) `xorUInt` fromConstant (snd t)) -- high word of the offset
               ]

      v0 =
        if lastBlock -- last block flag set ?
  ...
  let bb = value @bb'
      ...
      h' =
        h
          // [
               ( 0
               , (h ! 0) `xorUInt` fromConstant @Natural 0x01010000 `xorUInt` (fromConstant kk `shiftUIntR` 8) `xorUInt` fromConstant nn
               )
             ]
      ...
      h'' =
        if dd > 1
          then
            foldl' (\acc i -> blake2b_compress (Blake2bCtx acc (d !! i) (toOffset @Natural $ (i + 1) * bb)) False) h' [0 .. dd -! 2]
          else h'
      ...
      bs = reverseEndianness @64 $ concat @8 @64 $ Vec.unsafeToVector @8 $ map from $ toList h'''
