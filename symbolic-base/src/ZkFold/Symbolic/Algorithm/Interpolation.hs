...
import Data.Foldable (foldr1, length, toList)
import Data.Function (($), (.))
import Data.Functor (fmap, (<$>), (<&>))
import Data.List (lookup, replicate)
import Data.List.NonEmpty (NonEmpty)
import qualified Data.List.NonEmpty as NE
import Data.Maybe (Maybe (..), fromJust, fromMaybe, mapMaybe)
import Data.Semialign (Semialign, alignWith, zip, zipWith)
...
pushInterpolation src = prune . zip (fst <$> src) <$> branches (snd <$> src)
 where
  prune = NE.fromList . mapMaybe (\(a, m) -> (a,) <$> m) . toList
