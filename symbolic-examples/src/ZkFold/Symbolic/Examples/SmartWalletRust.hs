...
module ZkFold.Symbolic.Examples.SmartWalletRust
    ( expModContract
    , expModCircuit
    , expModSetup
    , expModProof
...
import qualified ZkFold.FFI.Rust.Poly as Rust
import qualified ZkFold.FFI.Rust.RustBLS as Rust
import qualified ZkFold.FFI.Rust.Types as Rust

-- Copypaste from zkfold-cardano but these types do not depend on PlutusTx
--
convertZp :: Rust.Fr -> Integer
convertZp = naturalToInteger . fromZp . r2h

convertG1 :: Rust.Rust_BLS12_381_G1_JacobianPoint -> ByteString
convertG1 = toByteString . compress . r2h

convertG2 :: Rust.Rust_BLS12_381_G2_JacobianPoint -> ByteString
convertG2 = toByteString . compress . r2h
...
type ExpModCircuit = ArithmeticCircuit Rust.Fr ExpModCompiledInput Par1

type PlonkupTs i n t = Plonkup i Par1 n Rust.Rust_BLS12_381_G1_JacobianPoint Rust.Rust_BLS12_381_G2_JacobianPoint t (Rust.RustPolyVec Rust.Fr)

type TranscriptConstraints ts =
    ( ToTranscript ts Word8
    , ToTranscript ts Rust.Fr
    , ToTranscript ts Rust.Rust_BLS12_381_G1_CompressedPoint
    , FromTranscript ts Rust.Fr
    )
...
expModCircuit = C.compile @Rust.Fr expModContract

expModSetup :: forall t .  TranscriptConstraints t => Rust.Fr -> ExpModCircuit -> SetupVerify (PlonkupTs ExpModCompiledInput ExpModCircuitGates t)
expModSetup x ac = setupV
    where
        (omega, k1, k2) = getParams (Number.value @ExpModCircuitGates)
        (gs, h1) = getSecretParams @ExpModCircuitGates @Rust.Rust_BLS12_381_G1_JacobianPoint @Rust.Rust_BLS12_381_G2_JacobianPoint x
...
expModProof x ps ac ExpModProofInput{..} = proof
    where
        ...
        witnessInputs :: ExpModLayout Rust.Fr
        witnessInputs = runInterpreter $ arithmetize input Proxy

        paddedWitnessInputs :: ExpModCompiledInput Rust.Fr
        ...
        witness = (PlonkupWitnessInput @ExpModCompiledInput @Rust.Rust_BLS12_381_G1_JacobianPoint paddedWitnessInputs, ps)
...
identityCircuit :: ArithmeticCircuit Rust.Fr Par1 Par1
identityCircuit = C.compileWith @Rust.Fr AC.solder (\i -> (U1 :*: U1, i :*: U1)) identityFun 

expModSetupMock :: forall t . TranscriptConstraints t => Rust.Fr -> SetupVerify (PlonkupTs Par1 ExpModCircuitGatesMock t)
expModSetupMock x = setupV
    where
        (omega, k1, k2) = getParams (Number.value @ExpModCircuitGatesMock)
        (gs, h1) = getSecretParams @ExpModCircuitGatesMock @Rust.Rust_BLS12_381_G1_JacobianPoint @Rust.Rust_BLS12_381_G2_JacobianPoint x
...
expModProofMock x ps ExpModProofInput{..} = proof
    where
        ...
        witnessInputs :: Par1 Rust.Fr
        witnessInputs = Par1 $ h2r input

        (omega, k1, k2) = getParams (Number.value @ExpModCircuitGatesMock)
        (gs, h1) = getSecretParams @ExpModCircuitGatesMock @Rust.Rust_BLS12_381_G1_JacobianPoint @Rust.Rust_BLS12_381_G2_JacobianPoint x
        plonkup = Plonkup omega k1 k2 identityCircuit h1 gs
        setupP  = setupProve @(PlonkupTs Par1 ExpModCircuitGatesMock t) plonkup
        witness = (PlonkupWitnessInput @Par1 @Rust.Rust_BLS12_381_G1_JacobianPoint witnessInputs, ps)
        (_, proof) = prove @(PlonkupTs Par1 ExpModCircuitGatesMock t) setupP witness

foreign export ccall mkProofBytesWasmR :: CString -> CString -> CString -> IO CString
foreign export ccall mkProofBytesMockWasmR :: CString -> CString -> CString -> IO CString

mkProofBytesWasmR :: CString -> CString -> CString -> IO CString
mkProofBytesWasmR xPtr psPtr proofInputPtr = do
...
mkProofBytesMockWasmR :: CString -> CString -> CString -> IO CString
mkProofBytesMockWasmR xPtr psPtr proofInputPtr = do
    (x, ps, proofInput) <- readPointers xPtr psPtr proofInputPtr
...
readPointers :: CString -> CString -> CString -> IO (Rust.Fr, PlonkupProverSecret Rust.Rust_BLS12_381_G1_JacobianPoint, ExpModProofInput)
readPointers xPtr psPtr proofInputPtr = do
    xStr <- peekCString xPtr
    let x = h2r $ toZp $ read xStr

    psStr <- peekCString psPtr
    let ps = PlonkupProverSecret $ V.unsafeToVector $ (h2r . toZp . read) <$> words psStr
